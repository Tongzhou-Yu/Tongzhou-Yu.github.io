	...
	void main(){
		...
		for(int i=0;i<${whirlCount};i++){
		  vec2 delta = st-whirls[i].xy;
			...
			float dr = (1.-smoothstep(0.,whirls[i].w,d))*(-1.+sin(d*5.+u_time))*(0.5+cos(d*40.+u_time))/2.;
			d+=dr;
			
			
			 // d+=cnoise(vec3(st*5.,u_time/200.))/50.;
			 // ang+=cnoise(vec3(st*5.,u_time/2000.))/300.;
			// ang+=sin(d*3.)/2.;
			// d+=sin(ang*2.)/5.;
		...
		vec3 color = vec3(sin(st.x),sin(st.x*1.1),sin(st.x*1.2));
